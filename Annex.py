...
class SettingWindow:
    ...
    def settingWindow(self,root):
        ...
        self.volume=ttk.Label(self.setting,text="Anni's Volume: ",borderwidth=0,font=('"Times New Roman"')).place(x=3,y=17)
...
class News:
    ...
    def show(self):
        ...
        r=requests.get('http://newsapi.org/v2/top-headlines?country=in&apiKey=d2fbd834d9b44559b7daebed7a3997c4')
...
class Weather:
    def show(self,scrollable_text):
        SR=SpeakRecog(scrollable_text)
        base_url = "http://api.openweathermap.org/data/2.5/weather?q=Visakhapatnam,IN&units=metric&appid=3079fdc2386dcd0f08f7c62a37c72daf"
        data=requests.get(base_url).json()
        SR.scrollable_text_clearing()
        SR.speak("-----------------------------Weather Report of VISAKHAPATNAM City------------------------------")
